...
const ListRoom = ()=>{
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:8000/api/list-rooms')
            .then(response => {
                setListRooms(response.data.listRooms);
                setIsLoading(false);
            })
            .catch(error => {
                setError(error);
    ...
    return (
        ...
        <div className="md:pt-[65px] pt-[60px]">
            <div className="bg-[#f5f5f5] lg:py-4 lg:pt-6 py-3 pt-2 ">
                ...
                <span className="mx-2 text-[#915b10] text-[14px] font-mono"><a href="#" >Tât cả các phòng</a></span>
            </div>
        </div>
